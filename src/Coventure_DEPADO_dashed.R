...
# THIS TABLE PROVIDES FREQUENCIES TO MAKE SURE THAT THE VARIABLE IS DASHED
table(cov_5$Flagged,cov_5$year,cov_5$Exp_Group)
...
saveRDS(model3a, "/models/depado_dashed/model3a.rds")
saveRDS(mod_subs_a, "/models/depado_dashed/mod_subs_a.rds")

##### model with categorical time variable without IPW #####
cov_5$year=as.factor(cov_5$year)
model3d <- brm(Flagged ~ 1 + DEM_01 + Exp_Group + Language + year + year:Exp_Group + (1|id+schoolnum),  
               data = cov_5, 
               family = bernoulli,
               warmup = 1000, iter = 3000, 
               cores = 4, chains = 4, 
...
# Saving the model and its summary using saveRDS
saveRDS(model3d, "/models/depado_dashed/model3d.rds")
saveRDS(mod_subs_d, "/models/depado_dashed/mod_subs_d.rds")
...
# Saving the model and its summary using saveRDS
saveRDS(model3v, "/models/depado_dashed/model3v.rds")
saveRDS(mod_subs_v, "/models/depado_dashed/mod_subs_v.rds")
...
# THIS TABLE PROVIDES FREQUENCIES TO MAKE SURE THAT THE VARIABLE IS DASHED
table(cov_5_b$Flagged,cov_5_b$year,cov_5_b$Exp_Group)

##### model with continuous time variable with IPW #####
cov_5_b$Flagged=as.factor(cov_5_b$Flagged)
model3b <-  brm(Flagged|weights(ipw) ~ 1 + DEM_01 + Exp_Group + Language + year + year:Exp_Group + (1|id+schoolnum),  
                data = cov_5_b, 
                family = bernoulli,
                warmup = 1000, iter = 3000, 
                cores = 4, chains = 4, 
...
# Saving the model and its summary using saveRDS
saveRDS(model3b, "/models/depado_dashed/model3b.rds")
saveRDS(mod_subs_b, "/models/depado_dashed/mod_subs_b.rds")
...
# Saving the model and its summary using saveRDS
saveRDS(model3c, "/models/depado_dashed/model3c.rds")
saveRDS(mod_subs_c, "/models/depado_dashed/mod_subs_c.rds")
