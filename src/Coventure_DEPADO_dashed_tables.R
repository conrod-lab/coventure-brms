...
# Create a table for Flagged variable frequencies
table_Flagged <- table(cov_5$Flagged, cov_5$year, cov_5$Exp_Group)

# Convert the table to a data frame for better formatting
table_df <- as.data.frame(table_Flagged)

# Create a kableExtra table for the frequencies
kable_freq <- kable(table_df, format = "html", caption = "Flagged Variable Frequencies") %>%
  kable_styling(full_width = FALSE)

# Save the kableExtra table to an HTML file
html_file_path_freq <- file.path("/home/spinney/scratch/coventure/models/depado_dashed/html_output", "Flagged_variable_frequencies.html")
save_kable(kable_freq, html_file_path_freq)
cat("HTML file saved:", html_file_path_freq, "\n")
